...
To implement the pipeline we will use ksqlDB. Go to your Confluent Cloud Cluster and select ksqDB on the left and click on ``ksql_cluster_0`` to access the database's query editor

image::ksqlDB-cluster.png[]
...
Let's create a table for listing each player's highest score.
